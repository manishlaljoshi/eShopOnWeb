...
public class GetOrderDetailsHandler : IRequestHandler<GetOrderDetails, OrderDetailViewModel?>
{
    ...
    public async Task<OrderDetailViewModel?> Handle(GetOrderDetails request, CancellationToken cancellationToken)
    {
        ...
        // Logical change: Return two additional properties from the OrderDetailViewModel
        return new OrderDetailViewModel
        {
            ...
            Total = order.Total(),
            TaxAmount = order.CalculateTax(), // Added logic to calculate tax amount
            TotalWithTax = order.Total() + order.TotalWithTax(), // Added logic to calculate total with tax
        };
    }
}